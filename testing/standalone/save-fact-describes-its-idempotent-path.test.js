/**
 * `saveFact`'s description agrees with `saveFact`'s code about whether an `id` updates anything.
 *
 * ## The defect
 *
 ...
 * update…')`, and `brain/fact.ts` has the branch to match — a supplied id that already names a record
 ...
 * the same wrong claim. The branch is read from `brain/fact.ts`, and the description is held to it.
 *
 * Run: node --test testing/standalone/save-fact-describes-its-idempotent-path.test.js
...
describe('the code really does converge on a supplied id', () => {
  ...
  it('and `saveFact` accepts an id at all', () => {
    const schema = saveFact().inputSchema(STUB);
...
describe('the description agrees with it', () => {
  it('does NOT claim there is no id to update', () => {
    ...
    const text = saveFact().description ?? '';
