/**
 * Phase 9.2 of the conversation extractor — the extraction is checked before a single record is written
 * (`F-31`, DECOMPOSITION.md 9.2, 9.3). Moved here from `benchmarks/writer/validate-extraction.mjs`, which now
 * re-exports this: the benchmark's writer and the product's `ingest` refuse the same files for the same reasons,
 * because there is one copy of the rules.
 *
 ...
 */
function startsWithSpeakerName(text: string, speaker: string) {
...
export function validateExtraction(extraction: any, schemaEntries: SchemaEntry[]): string[] {
  const problems: string[] = [];
  const say = (m: string) => problems.push(m);

  if (!extraction || typeof extraction !== 'object') return ['the extraction is not an object'];

  const typesOf = (kind: string) => new Set(schemaEntries.filter(e => e.knowledgeType === kind).map(e => e.typeName));
  ...
  const defineKey = (key: string, kind: string, at: string) => {
  ...
  for (const [i, c] of chrono.entries()) {
    ...
    if (!CHRONO_STATUSES.includes(c.status)) {
      const retired = (RETIRED_CHRONO_STATUSES as Record<string, string>)[c.status];
      say(retired
   ...
   */
  const checkSupersedes = (e: any, at: string) => {
   ...
   */
  const declaredTurns = sessions.flatMap((s: any) => s.turns ?? []);
  ...
  if (declared.length > 0) {
    const covered = new Set(claims.flatMap((c: any) => c.sourceTurns ?? []));
    const missing = declared.filter((t: string) => !covered.has(t));
    if (missing.length > 0) {
 ...
 */
export function assertWritable(extraction: any, schemaEntries: SchemaEntry[]): void {
