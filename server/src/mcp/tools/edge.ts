...
export const graph_traverseTool: ToolHandler = {
  ...
  description: 'Follow edges from a starting entity and return reachable nodes up to `maxDepth` hops. For dependency analysis, impact assessment and lineage.\n\n'
    ...
    + 'THE RESPONSE: `nodes` — each with `id`, `name`, `type`, `kind` ("entity", or the collection it lives in when it is a chrono entry, fact or file — reached either through an explicit edge or through one of the include flags) and the `depth` it was found at, `startId` itself at depth 0. `edges` — the connecting relationships, unless `includeEdges` is false. `truncated` — true when `limit` cut the walk, and worth reading: a truncated walk is a PARTIAL graph, so an impact assessment run on one is answering a smaller question than it was asked.',
  spaceRequired: true,
  inputSchema: (s: ToolSchemas) => ({
          type: 'object',
          properties: {
            space: s.requiredSpace,
            startId: { type: 'string', minLength: 1, description: 'UUID of the starting record — usually an entity, but a fact or chrono entry works too, since an edge may name one at either end. It is returned as the first node at depth 0, so a walk that finds nothing still comes back with one node rather than empty — an empty `nodes` means the id resolved to nothing, which is a different answer from "it has no neighbours".' },
            direction: {
              type: 'string',
