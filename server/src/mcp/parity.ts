...
export const REST_ONLY_CAPABILITIES: readonly RestOnlyCapability[] = [
  ...
  },
  {
    ...
    why: 'governing a network is only partly on MCP: an agent can read, create, update and leave one and add a space to it (`network_get`, `network_create`, `network_update`, `network_leave`, `network_add_space`), see peers and trigger a sync — and cannot join or fork one, manage its members, invite, or, the one that matters, VOTE. A vote is how a networked space approves a destructive act, so an agent can be a member of a governance process it cannot take part in.',
  },
  {
