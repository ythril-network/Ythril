...
| 🎛️ **Recall you can actually tune** | A filter that runs INSIDE the vector index rather than after it, so nothing is silently dropped by a `topK`. Per-type quotas (`minPerType` / `maxPerType`) so one noisy type cannot crowd out the rest. A time budget (`maxTimeMS`) that returns what it has with a `degraded` flag instead of hanging or erroring. A record written seconds ago and not yet vector-indexed is still found, with no parameter to remember: every recall also reads the newest records straight from the collection. Field projection applied recursively through traversal results. And the lexical, fused and rerank scores on request, so a bad ranking can be diagnosed rather than guessed at. |
