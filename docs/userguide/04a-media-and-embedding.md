...
The **External assist model** card lets you point a bigger, hosted model at the **document repair pass** — the one used by the `repair` extraction level (and by `auto` when a repair model is configured). It does two jobs, and **each is consented on its own**. The document repair pass is switched on by the **extraction level**, and asks for the documents acknowledgement below. **Conversations** — when one is ingested, this model writes its claims and entity descriptions, and answers the extractor's questions if no Decision model is set — are allowed with the card's **Allow conversations** button, which opens a dialog naming what goes to the host: the conversation's turns, who said what and when. A documents acknowledgement never covers conversations, and changing the endpoint withdraws both. **Withdraw conversations** takes it back; either takes effect when you save the card. Ingest refuses a raw conversation until conversations are allowed, and says so. Fill in an **Endpoint** + **Model**, then raise **Document extraction** to **Repair** (or **Auto**) to route repairs through it. At that point you are asked to **acknowledge the egress** — document content (OCR text, and page images) is sent to that host. The acknowledgement is recorded against the host and re-checked at run time, so an endpoint you have not acknowledged is never contacted: repairs fall back to the local model instead.
