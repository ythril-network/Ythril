...
- Chrono entries with `type: "event"` timestamp every incident. `query(chrono, {entityName: "payment-service", type: "event"})` → "payment-service has had 4 incidents this quarter" — pattern detection for free.
