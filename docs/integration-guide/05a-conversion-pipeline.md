...
| `api` | Which API the endpoint speaks: `openai` (the default — OpenAI-compatible, as above) or `anthropic`, the **Claude API** (`POST {baseUrl}/v1/messages` through the official SDK, the key sent as `x-api-key`). The Claude API is used with an **API key** from the Claude Console; a Claude.ai or Claude Code subscription is not a credential for it. No `temperature` is sent to Claude, and its `input_tokens` / `output_tokens` are what the budget is charged. |
| `budget` | `{ "tokens": N, "perHours": H }` — at most `N` tokens in any rolling `H` hours (1 to 720), charged from each reply's reported usage, estimated from the characters when a reply reports none. Once spent, calls go to `fallback` (or, with none, to whatever runs without an assist model) until the window rolls on. It binds this endpoint only. **Absent keeps what is stored, `null` removes it** — unlike the rest of this block, so a client that has never heard of it cannot drop it. |
| `fallback` | The endpoint that answers when this one may not: unreachable, rate-limited or overloaded (`429`, `529`, a `5xx` — the primary is then passed over for a minute), over its budget, not consented for the use, or declining a request (a Claude refusal, which does not pause the primary). `{ api, baseUrl, model, apiKey, acknowledgedHost, acknowledgedHostForConversations }`, the same fields as here. A **local** fallback — loopback or a bare service name such as `http://ollama:11434/v1` — sends nothing off the instance and needs no consent; any other URL is SSRF-checked and consented per use like the primary. Its key is stored as `docAssistFallbackApiKey` in `secrets.json`, masked on GET, env `DOC_ASSIST_FALLBACK_API_KEY`. Absent keeps, `null` removes. `GET /api/admin/pipeline-status` reports it as the `assist-fallback` stage, and its `assist` block says which endpoint answers each use now, what the window has spent, and whether the primary is paused; `POST /api/admin/media-config/test-connection` and `/verify` take `"fallback": true` with `"target": "assist"` to probe it. |
