...
| Network | `network.create`, `network.update`, `network.delete`, `network.space.add` (the space list before and after), `network.space.pending` (a space the network proposed, accepted or dismissed: the space list, the pending list and the dismissed list before and after), `space.precedence.update`, `network.join`, `network.join_remote`, `network.join_by_key` (this instance joining a pub/sub by its published key), `network.fork`, `network.vote`, `network.invite`, `network.member.add`, `network.member.remove`, `network.member.adopt`, `network.member.signing_key`, `network.member.revert_parent`, `network.reparent_self`, **`network.invite.generate`** (an admin producing join material), **`network.member.join`** (the moment another instance becomes a member of a network on this one — the invite handshake's `finalize`), **`network.sync_trigger`** (a sync of every peer in a network) and **`peer.sync_trigger`** (a sync of one named peer) |
