...
| `modelSlots.<slot>.timeoutMs` | *(none — see below)* | per slot | **How long ONE call to a model slot may take.** The slots are `vision`, `stt`, `embedding`, `rerank`, `nli`, `assist`, `docVlm`, `docRepair`, `docVerify`, `faceExternal` and `decision` — the same names the per-slot egress permissions use. Defaults are the values these calls always had: vision 120 s, stt 300 s, embedding 30 s, rerank 20 s, nli 20 s, faceExternal 30 s, and 60 s for the four document slots and for `decision`. Absent means the default; `null` through the admin PATCH clears it back to the default. Range 1 000 … 1 800 000 ms. **Deliberately no environment variable** — every setting that had both an env var and an admin field was found to have two different legal ranges, so these have one door. Infra pins a slot with `YTHRIL_PINNED_FIELDS=modelSlots.vision`, which fixes it at whatever the config resolves to. **Raising one raises the stall floor with it**: the media stall detector is fed the configured value, so a long call is not re-queued while it is still running. **On the four document slots** — `docVlm`, `docRepair`, `docVerify` and `assist` — a value set here wins over `documentProcessing.pageTimeoutMs`, which is what those calls fall back to when no slot budget is set. |
...
### The extractors' decision model (`decisionModel`)

The conversation extractor (`F-31`, being built) asks a model its judgement questions — *which of these
people is "she"*, *is this turn pasted material*, *does this claim already exist* — as typed questions over a
state, and does everything else in code. This is the endpoint it asks.

| Field | Env | Default | |
|---|---|---|---|
| `decisionModel.baseUrl` | `DECISION_URL` | `https://api.typesafe.ai` | A [System One](https://docs.typesafe.ai/api.md) endpoint: `POST <baseUrl>/v1/systemone`. A base with or without `/v1` lands on the same URL. |
| `decisionModel.model` | `DECISION_MODEL` | `jev-latest` | |
| key | `DECISION_API_KEY` | — | Stored in `secrets.json` as `decisionApiKey`, never in `config.json`, and returned masked. |
| `decisionModel.acknowledgedHost` | — | — | **Consent to send conversation text to that host.** Config only: infrastructure can pin the endpoint but never consent for the operator. |

- **Nothing is sent until the host is acknowledged**, and the check runs at the call as well as on save, so a
  hand-edited `config.json` cannot skip it. `PATCH /api/admin/media-config` with `{"decisionModel": {...}}`
  refuses a patch that sets the endpoint up without consent, with `needsAcknowledgment: <host>` in the body.
  Removing the key or withdrawing consent (`acknowledgedHost: ""`) needs no consent.
- **The fallback is the assist model** (`documentProcessing.assistModel`). Without consent to the decision
  host, the same questions go to the assist model as an OpenAI chat call whose JSON schema only admits the
  listed options. It answers with a pick, not a distribution, so its `probabilities` are `null`, and every
  answer records which backend gave it.
- **With neither, extraction is refused up front**, naming both settings — never guessed.
- **Every answer is checked by code** whichever backend gave it: a choice outside the options, a number
  outside its range or a missing answer is marked `invalid` with the reason, and every choice question must
  offer a no-match option (`none`, `new`, `neither` or `unclear`) or it is refused before it is sent.
- Any of the three env vars locks the block (`403` on a PATCH, and the Models tab card is read-only). Its
  call budget is `modelSlots.decision`, and its private-address permission `YTHRIL_ALLOW_PRIVATE_DECISION`.
